...
module hls_sim_mm_master_dpi_bfm(
                                  // clock, reset
                                  clock,
                                  reset_n,
                                  ...
                                  // CRA Slave (if it exists)
                                  done_writes_to_cra,
                                  slave_memory_writes_done,
   ...
   parameter COMPONENT_SLAVE_WRITE_INTERFACE_NAME = "__ihc_hls_avs_write_stream__";
   parameter COMPONENT_SLAVE_READ_INTERFACE_NAME  = "return";
   parameter COMPONENT_CRA_SLAVE                  = 1; // 0 - slave memory with base address 0 (only data in the write stream , so calculate the address in this IP)
                                                       // 1 - this is the CRA slave and the write stream contains additional information (address, byte-enable, etc.)
   parameter NUM_SLAVE_MEMORIES                   = 0; // If this is the CRA slave, we need to know how many slave memories to wait for
   ...
   // The slave write stream consists of a series of write commands packed as {lastwrite, byteenable, data, address}
   // Where last write indicates that it is now safe to start the component from the component's DPI controller
   localparam SLAVE_WRITE_STREAM_ADDRESS_WIDTH    = 64;
   localparam SLAVE_WRITE_STREAM_DATA_WIDTH       = AV_SYMBOL_W*AV_NUMSYMBOLS;
   localparam SLAVE_WRITE_STREAM_BYTEENABLE_WIDTH =  AV_NUMSYMBOLS; // AV_SYMBOL_W is always 8
   localparam SLAVE_WRITE_STREAM_WIDTH = (COMPONENT_CRA_SLAVE == 1) ? SLAVE_WRITE_STREAM_ADDRESS_WIDTH + SLAVE_WRITE_STREAM_DATA_WIDTH + SLAVE_WRITE_STREAM_BYTEENABLE_WIDTH :
                                                                      SLAVE_WRITE_STREAM_DATA_WIDTH;

   import "DPI-C" context function void __ihc_hls_dbgs(string msg);
   import "DPI-C" context function int __ihc_hls_get_slave_return_info(string component_name, string interface_name, inout bit [SLAVE_WRITE_STREAM_ADDRESS_WIDTH-1:0] done_address, inout bit [SLAVE_WRITE_STREAM_ADDRESS_WIDTH-1:0] data_address);
   ...
   input  [lindex(NUM_SLAVE_MEMORIES):0]            slave_memory_writes_done;
  ...
  function automatic write_to_slave(input bit [63:0] addr, input bit [63:0] data, input bit [7:0] byteenable);
  ...
  logic            [SLAVE_WRITE_STREAM_WIDTH - 1 : 0] input_stream_data;
  logic    [SLAVE_WRITE_STREAM_ADDRESS_WIDTH - 1 : 0] wr_addr;
  logic       [SLAVE_WRITE_STREAM_DATA_WIDTH - 1 : 0] wr_data;
  logic [SLAVE_WRITE_STREAM_BYTEENABLE_WIDTH - 1 : 0] wr_byteenable;
  logic                                               wr_last;

  logic wr_valid;
  logic ready_for_write;
  logic [lindex(NUM_SLAVE_MEMORIES):0] slave_memory_writes_done_r;

  hls_sim_stream_source_dpi_bfm
  #(  .STREAM_DATAWIDTH   (SLAVE_WRITE_STREAM_WIDTH),
      .COMPONENT_NAME     (COMPONENT_NAME),
      .INTERFACE_NAME     (COMPONENT_SLAVE_WRITE_INTERFACE_NAME))
  slave_input_stream
  (
  ...
  generate
    if (COMPONENT_CRA_SLAVE == 1) begin
      assign wr_addr       = input_stream_data[SLAVE_WRITE_STREAM_ADDRESS_WIDTH - 1 : 0];
      assign wr_data       = input_stream_data[SLAVE_WRITE_STREAM_ADDRESS_WIDTH + SLAVE_WRITE_STREAM_DATA_WIDTH - 1 : SLAVE_WRITE_STREAM_ADDRESS_WIDTH];
      assign wr_byteenable = input_stream_data[SLAVE_WRITE_STREAM_ADDRESS_WIDTH + SLAVE_WRITE_STREAM_DATA_WIDTH + SLAVE_WRITE_STREAM_BYTEENABLE_WIDTH - 1 : SLAVE_WRITE_STREAM_ADDRESS_WIDTH + SLAVE_WRITE_STREAM_DATA_WIDTH];
    end else begin

      logic [SLAVE_WRITE_STREAM_ADDRESS_WIDTH-1:0] addr_r;
      logic addr_inc;

      always @(posedge clock or negedge reset_n) begin
        if (~reset_n | wr_last) begin
          addr_r <= {SLAVE_WRITE_STREAM_ADDRESS_WIDTH{1'b0}};
        end else begin
          addr_r <= addr_r + {{(SLAVE_WRITE_STREAM_ADDRESS_WIDTH-AV_NUMSYMBOLS-1){1'b0}}, addr_inc, {$clog2(AV_NUMSYMBOLS){1'b0}}};
        end
      end
  ...
  assign wr_last = (COMPONENT_CRA_SLAVE == 1) ? (wr_valid & (input_stream_data === {SLAVE_WRITE_STREAM_WIDTH{1'b0}}) ) : // for the CRA slave, the last write is encoded as all 0's
  ...
  always @(negedge clock) begin
    if (enable & wr_valid & ready_for_write & !wr_last) begin
      write_to_slave(wr_addr, wr_data, wr_byteenable);
    end
  end
  ...
  always @(posedge clock or negedge reset_n) begin
    if (!reset_n) begin
      ...
      if (do_bind) begin
        done_writes <= 1'b0;
      end else if ((COMPONENT_CRA_SLAVE == 1) && component_started) begin
  ...
  // used for slave memories to signal CRA if it exists
  assign done_writes_to_cra = done_writes;


  // If this is the CRA slave, we need to wait for the non-CRA slave memories to finish their writes before we write it to prevent the start bit from being written too early
  // else just write as soon as the component and testbench are ready
  always @(posedge clock or negedge reset_n) begin
    if (!reset_n) begin
      slave_memory_writes_done_r <= 'b0;
    end else begin
      slave_memory_writes_done_r <= (wr_last === 1'b1) ? 'b0 : (slave_memory_writes_done | slave_memory_writes_done_r);
    end
  end

  typedef enum logic [1:0] {STATE_READY_FOR_WRITES, STATE_WRITES_DONE, STATE_WAIT_FOR_SLAVES_DONE} states_t;
  states_t slave_write_state;
  ...
  generate
    if (COMPONENT_CRA_SLAVE == 1) begin
      ...
      always @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
          slave_write_state <= STATE_WAIT_FOR_SLAVES_DONE;
        end else begin
          case (slave_write_state)
            STATE_READY_FOR_WRITES:     begin
                                          if (wr_last === 1'b1) begin
                                            slave_write_state <= STATE_WRITES_DONE;
                                          end
                                        end
            STATE_WRITES_DONE:          begin 
                                          if (!wr_valid ) begin
                                            ...
                                            slave_write_state <= STATE_READY_FOR_WRITES;
                                          end
                                        end
            STATE_WAIT_FOR_SLAVES_DONE: begin
                                          if ((NUM_SLAVE_MEMORIES == 0) | (&slave_memory_writes_done_r)) begin
                                            slave_write_state <= STATE_READY_FOR_WRITES;
                                          end
                                        end
            default: slave_write_state <= STATE_WAIT_FOR_SLAVES_DONE;
          endcase
        end
      end
    end else begin
      ...
      always @(posedge clock or negedge reset_n) begin
        if (!reset_n) begin
          ...
          case (slave_write_state)
            STATE_READY_FOR_WRITES: if (wr_last === 1'b1)  slave_write_state <= STATE_WRITES_DONE;
            STATE_WRITES_DONE:      if (component_started) slave_write_state <= STATE_READY_FOR_WRITES;
          endcase
        end
      end
    end
  ...
  assign ready_for_write = (slave_write_state == STATE_READY_FOR_WRITES) | wr_last_sanitized; //always ready out last write from slave stream
  ...
  generate
    ...
    logic [SLAVE_WRITE_STREAM_ADDRESS_WIDTH - 1 : 0] done_baseaddr;
    logic [SLAVE_WRITE_STREAM_ADDRESS_WIDTH - 1 : 0] returndata_baseaddr;
    logic    [SLAVE_WRITE_STREAM_DATA_WIDTH - 1 : 0] returndata;
    logic                                            return_ready;
    logic                                            return_valid;
    logic                                            write_return;
    ...
    //    and the function invocation type is not a slave
    if ((COMPONENT_CRA_SLAVE == 0) || (COMPONENT_HAS_SLAVE_RETURN == 1)) begin
      hls_sim_stream_sink_dpi_bfm
      #(  .STREAM_DATAWIDTH   (SLAVE_WRITE_STREAM_DATA_WIDTH),
          .COMPONENT_NAME     (COMPONENT_NAME),
          .INTERFACE_NAME     (COMPONENT_SLAVE_READ_INTERFACE_NAME))
      ...
      task read_all_return_data (input bit [63:0] done_addr, input bit [63:0] data_addr);
        ...
        for (i = 0; i < num_registers; i++) begin
          // Read slave memory through BFM
          read_from_slave(data_addr + (i << $clog2(AV_NUMSYMBOLS)));
          @(posedge clock);
        end
        ...
        if ((COMPONENT_CRA_SLAVE == 1) && (COMPONENT_HAS_SLAVE_RETURN == 1)) begin
          read_from_slave(done_addr);
          @(`MSTR_BFM.signal_all_transactions_complete);
          `MSTR_BFM.pop_response();
          returndata = `MSTR_BFM.get_response_data(0);
          @(posedge clock);
      ...
      initial begin
        ...
        forever begin
          ...
          num_registers = __ihc_hls_get_slave_return_info(COMPONENT_NAME, COMPONENT_SLAVE_READ_INTERFACE_NAME, done_baseaddr, returndata_baseaddr);
