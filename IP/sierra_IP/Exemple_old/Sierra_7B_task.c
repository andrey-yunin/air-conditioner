...
- Timer_task_10: print every 10 secound a "A".
- Timer_task_0_5: print every half second a "b".
- Configuration file
- Added deadline control, same as the periodic time
...
void timer_task_code(void)
{
    ...
    while(1)
    {
      ...
      if (test.periodic_start_integer & 0x1) printf("\ndeadline miss, timer task");
      for(i=0; i<5000; i++);  //virtual load
 //     for(i=0; i<500000; i++);  //extra virtual load
 //     for(i=0; i<500000; i++);  //extra virtual load
      temp = temp +1;
      printf("\nSec: %d ", temp);
    }
...
void timer_task_10_code(void)
{
    ...
    {
      test = wait_for_next_period(); //Every secound
      if ((test.periodic_start_integer & 0x1) != 0) printf("\ndeadline miss, timer 10 task");
